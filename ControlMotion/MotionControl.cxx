...
// constructor
MotionControl::MotionControl(Entity* e, const char* part, const
			     PrioritySpec& ps) :
...
bool MotionControl::completeSound()
{
  if (next_motion_name.size()) {
    if (next_coordinates.size() == 12) {
      ...
      next.setquat(next_coordinates[3],
                   next_coordinates[4],
                   next_coordinates[5]);
      ...
      next.dt = next_dt;
      next.volume = 1.0;
      motion_sets.push_back
        (boost::shared_ptr<MotionSet>
...
MotionControl::MotionSet::MotionSet
(const AudioObjectMotion& i, const GlobalId &master_id,
  ...
  moving(i)
{
  w_entity.reset(new ChannelWriteToken
                 (master_id, NameSet("audio", "AnyAudioClass", ""),
                  "AudioObjectMotion", name,
...
MotionControl::FixedSet::FixedSet
(const AudioObjectFixed& i, const GlobalId &master_id,
 const std::string name, int eventinterval) :
  name(name),
  eventinterval(eventinterval),
  eventcounter(eventinterval),
...
bool MotionControl::isPrepared()
{
  ...
  for (fixed_sets_t::iterator ii = fixed_sets.begin();
       ii != fixed_sets.end(); ii++) {
    res = res && (*ii)->isValid();
  }
